...
const linc_state_types_1 = require("@CmTypes/linc.state.types");
const linc_queued_message_1 = require("@shared/linc.queued-message");
...
class LincSocket {
    ...
    setupBrowserEvents() {
        if (!this.socket)
            return;
        this.socket.onopen = () => {
            this.state = linc_state_types_1.LincState.Open;
            if (this.onOpen)
                this.onOpen();
        };
        ...
        this.socket.onclose = (event) => {
            this.state = linc_state_types_1.LincState.Closed;
    ...
    async sendAwait(data) {
        if (this.readyState === WebSocket.OPEN) {
            ...
            return new Promise((resolve, reject) => this.messageQueue.add(new linc_queued_message_1.QueuedMessage(data, resolve, reject)));
        }
        else {
